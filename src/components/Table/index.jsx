...
export default function Table({ columns, dataSource, heading, adding }) {
  return (
    <>
            ...
            <tbody className="bg-white">
              {dataSource?.map(data => (
                <tr key={data.key}>
                  {columns?.map(col => (
                    <td
                      key={col.key + data.key}
                      className="py-4 whitespace-no-wrap border-b border-gray-500 text-blue-600"
                    >
